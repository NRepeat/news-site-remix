...
export default function ConstructorComponent({
	...
	return <div onMouseEnter={() => setOpacity(true)} onMouseLeave={() => setOpacity(false)} className={styles.container}>{elementInstance.additionalProperties?.label}
		<ConstructorElementEditBar opacity={opacity} elementInstance={elementInstance} slug={page?.slug} />

		<CustomSlider >
			{Array.isArray(content) && content.map(img => <Slide key={img.filepath} img={img.name} />)}
		</CustomSlider>
	</div>;
}